...
def get_photo():
	...
	try:
		...
		''' REMEMBER: Right now we are:
						1) Only using depression right now
		...
		q2 = query['validate']['cesd'].format(cutoff=cesd_cutoff, 
											  names=unames)
...
@app.route("/addrating/<rater_id>/<happy>/<sad>/<likable>/<interesting>/<one_word>/<description>/<encoded_url>/<epsilon>", methods=['POST', 'GET', 'OPTIONS'])
@crossdomain(origin='*')
def add_rating(rater_id,happy,sad,likable,interesting,one_word,description,encoded_url,epsilon,table="photo_ratings"):

	# epsilon just avoids CORS issues from qualtrics
	...
	try:
		...
		log_msgs.append('\Rating for url: {} [rater id: {}] stored successfully!'.format(url,values[0]))
		util.log(log_msgs,log_dir,full_path_included=True)
		return query2
	except Exception,e:
		log_msgs.append('\Error recording rating for url: {} [rater id: {}] [ERROR: {}]'.format(url,values[0],str(e)))
		util.log(log_msgs,log_dir,full_path_included=True)
		return str(e)
